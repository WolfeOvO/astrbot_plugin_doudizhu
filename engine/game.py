...
class GameState:
    ...
    def __init__(self, uids_names: Sequence[Tuple[str, str]], mode: str = "classic",
        ...
        self.log.append(f"牌局开始（{'经典' if mode == 'classic' else '癞子'}场）"
    ...
    def double(self, seat: int, do_double: bool) -> str:
        ...
        if do_double:
            ...
            self.multiplier *= 2
            self.log.append(f"{name} 加倍（倍数 x2 → {self.multiplier}）")
            msg = f"{name} 加倍！✕2"
        else:
    ...
    def play(self, seat: int, cards: Sequence[str]) -> Tuple[P.Combo, str]:
        ...
        if not p.hand:
            ...
            return combo, f"{p.name} 出完了！"
        self.current = (seat + 1) % 3
        return combo, f"{p.name} 出 {combo.text()}"
    ...
    def pass_turn(self, seat: int) -> str:
        ...
        if self.pass_count >= 2:
            ...
            return f"{p.name} 不出（两家不出，{self.players[last.seat].name} 重新出牌）"
        self.current = (seat + 1) % 3
        return f"{p.name} 不出"
    ...
    def settlement_text(self) -> str:
        ...
        lines.append(f"【结算】{'地主' if lw else '农民'}胜利"
                     f"｜底分 {self.base_points} × 倍数 {self.multiplier}"
                     f"（含炸弹 {self.bomb_count} 个）")
        if self.spring_type == "spring":
            lines.append("🌸 春天翻倍")
        elif self.spring_type == "anti":
            lines.append("🌸 反春天翻倍")
        for p in self.players:
            ...
            lines.append(f"· {p.name}（{role}）：{delta} 乐豆")
        return "\n".join(lines)
